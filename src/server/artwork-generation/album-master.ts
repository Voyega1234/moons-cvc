...
export function buildAlbumMasterInstruction(
  ...
  return [
    ...
    "Keep text, logo, CTA, faces, products, and essential proof at least 8% inside each panel boundary. Never place essential content across a separator. This is a hard physical constraint, not a stylistic preference: the panel boundary is where this artwork gets mechanically cropped into a separate file, so anything closer than 8% to it — especially a bottom-anchored icon row, caption line, or feature strip — will be sliced through and appear as a broken, half-visible element in the final asset. Give any bottom-anchored row of icons, captions, or feature strips real breathing room above the boundary; shrink it or move it up rather than letting it hug the edge.",
