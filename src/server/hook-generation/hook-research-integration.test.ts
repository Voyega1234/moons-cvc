  ...
  it("researches once, then sends the evidence dossier to a tool-free Hook Agent", async () => {
    ...
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(
        jsonResponse({
          summary: "ธุรกิจต้องการหลักฐาน visibility ที่ตรวจสอบได้",
          references: [
            {
              id: "ref-01",
              title: "Thai Consumer Report",
              content: "ผู้ใช้เริ่มใช้ AI เพื่อค้นหาข้อมูลธุรกิจ",
          ...
          insights: [
            {
              title: "ลูกค้าเปลี่ยนวิธีค้นหา แต่หลายแบรนด์ยังวัด visibility แบบเดิม",
              content: "ผู้ใช้เริ่มใช้ AI เพื่อค้นหาข้อมูลธุรกิจ ทำให้การติดอันดับ Search เดิมอาจไม่พอ เจ้าของธุรกิจอาจไม่เห็นช่วงที่ AI แนะนำคู่แข่ง และ Webinar เชื่อมกับการตรวจ AI visibility ได้",
              referenceIds: ["ref-01"]
            }
          ],
          gaps: []
    ...
    expect(JSON.stringify(hookBody.input)).toContain(
      "Start creative reasoning from insights"
    );
    expect(JSON.stringify(hookBody.input)).toContain(
