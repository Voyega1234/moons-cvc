...
function toFetchRequest(request: VercelRequest): Request {
  return new Request("https://moons.local/api/hook-generation-harness", {
...
function parseJsonBody(text: string): unknown {
  if (!text.trim()) {
    return {
      ok: false,
      error: "Hook generation harness returned an empty response body."
    };
  }
  ...
  try {
    ...
    return {
      ok: false,
      error: "Hook generation harness returned a non-JSON response body."
    };
  }
}
