...
export default async function handler(
  ...
  const workerResponse = await handleAnalyzeGuidelineRequest({
    request: toFetchRequest(request),
    env: {
      OPENAI_API_KEY: process.env.OPENAI_API_KEY,
      OPENAI_GUIDELINE_ANALYSIS_MODEL: process.env.OPENAI_GUIDELINE_ANALYSIS_MODEL,
      OPENROUTER_API_KEY: process.env.OPENROUTER_API_KEY,
      OPENROUTER_GUIDELINE_ANALYSIS_MODEL:
        process.env.OPENROUTER_GUIDELINE_ANALYSIS_MODEL,
...
function parseJsonBody(text: string): unknown {
  if (!text.trim()) {
    return {
      ok: false,
      error: "Guideline analysis returned an empty response body."
    };
  }
  ...
  try {
    ...
    return {
      ok: false,
      error: "Guideline analysis returned a non-JSON response body."
    };
  }
}
