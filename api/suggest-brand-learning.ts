...
export default async function handler(
  ...
  const workerResponse = await handleSuggestLearningRequest({
...
function parseJsonBody(text: string): unknown {
  if (!text.trim()) {
    return {
      ok: false,
      error: "Brand learning suggestion returned an empty response body."
    };
  }
  ...
  try {
    ...
    return {
      ok: false,
      error: "Brand learning suggestion returned a non-JSON response body."
    };
  }
}
